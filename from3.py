...
def plot_stock():
    ...
    # Highlight green-red-green data 上漲型態的畫上紅色,下跌型態的畫上綠色
    for i in range(1, len(df)-1):
        if (df['Close'][i-1] < df['Close'][i] < df['Close'][i+1]) and (df['Open'][i-1] < df['Open'][i] > df['Open'][i+1]):
            ax0.axvspan(i-1, i+1, alpha=0.3, color='red')
        elif (df['Close'][i-1] > df['Close'][i] > df['Close'][i+1]) and (df['Open'][i-1] > df['Open'][i] < df['Open'][i+1]):
            ax0.axvspan(i-1, i+1, alpha=0.3, color='green')
